/*
	...
	2.
		...
		2.DOM객체.addEventListener( '이벤트명' , ( )=>{ })		 // 익명 화살표 함수 정의해서 적용
		3.DOM객체.addEventListener( '이벤트명' , function ( ){ })// 익명 함수 정의해서 적용
		
		
	이벤트명
		1. DOMContentLoaded : HTML이 완전히 열렷을 때 [모두 로드되었을 때] 실행되는 이벤트
		2. window.onload	: 해당 객체가 호출 되었을때
		3. click			: 해당 객체를 클릭했을 때
		4. keyup			: 해당 객체에서 키보드를 누르고 떼어있을 때
...
moving.style.position = 'absolute';// 2. style 속성 이용한 css 추가
	...
	// 1. DOM객체명.style = `속석명 : 값 , 석성명 : 값 `
	// 2. DOM객체명.style.css속성명 = '값'; 주의할점 : css속성명 작성시 카멜표기법 사용
		//DOM객체명.style = 'background-color = blue';
		//DOM객체명.style.backgrountColor = 'blue';
		
	// * 버튼 위치의 초기값 x=가로축 y=세로출 block = 이동단위[30px씩 이동]
	let x = 7; y = 0; let block = 30;
	// 1. 버튼 위치 출력 함수
	movingPrint(); // JS 열렸을 때 최초 1회 실행
