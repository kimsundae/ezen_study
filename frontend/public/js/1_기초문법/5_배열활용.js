...
//2. 이벤트 : 무엇인가가 실행/일어 났음
	...
	// 이벤트 함수[코드들이 모여있는 주머니/공간]
		...
		// 함:상자 수:숫자 => 숫자들이 들어있는 상자 => 미리 정의 된 수
function 추가함수(){ // f start
	...
	// 3. 입력받은 데이터를 배열에 추가
	let search = 학생리스트.indexOf(name) != -1 ? alert('이미 존재하는 학생명입니다[수정불가]') : name
	학생리스트.push( name )
			// 4. 배열명.push(데이터)	: 해당 데이터를 배열에 추가
	// 4. 배열 출력
		// document.write( 학생리스트 )	//문서에 해당 데이터 쓰기(데이터 사라짐)
		// 1. ul 객체 호출
...
function 삭제함수(){	// f s
	...
	// * 유효성 검사 : 데이터가 개발자가 원하는 데이터인지 검사하는 과정 
	index != -1 ? 학생리스트.splice( index , 1 ) : alert('없는 데이터입니다.[삭제 불가]')
	// 5. 배열 출력
	let slistUl = document.querySelector('.slist')
	slistUl.innerHTML = '<li>'+학생리스트+'</li>'
	// * 현재 <input> 객체 입력값 초기화
