...
public class Board {
	 // ----------객체 ----------//
	   ...
	   // 1. 필드 : 객체의 데이터 저장하는 곳
	      /*
	          - 인스턴스필드 : 객체 생성 시 객체 안에 저장되어 있는 필드
	           - 정적필드      : [static] 프로그램 실행 시 우선 할당 객체 없이 저장되어 있는 필드
	   ...
	   // 2. 생성자 : 객체 초기화 할 때 사용
	      /*
	          ...
	          - 기타 키워드
	             ...
	             - this.필드명      : 외부로부터 들어온 매개변수명과 현재 클래스내 필드명과 같을 때 필드를 식별할 때 
	             - this()      : 현재 클래스의 생성자 호출
	...
	/*
	  	...
	  	- 메소드 선언
	  		접근제한자 리턴타입 메소드명( 타입 매개변수 , 타입 매개변수 ) {}
	  			...
	  			- 매개변수		: 메소드 호출 시 메소드 안으로 들어오는 변수
	  			- { }		: 메소드 실행코드
	  			- return	: ( void일 때 생략가능) 리턴타입에 맞는 값을 리턴 후 메소드 종료
	  		
	  	- 메소드 사용목적
	  		- 1. 객체간의 상호작용	:	서로 다른 객체간의 데이터 이동
	  		- 2. 재활용			: 코드 줄이기 / 클린 코딩
	  		- 3. 매개변수에 따른 서로 다른 실행/결과 반활할 수 있다.
