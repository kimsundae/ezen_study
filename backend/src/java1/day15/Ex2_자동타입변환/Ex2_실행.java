...
public class Ex2_실행 {
	...
	// 2. 자식객체 생성 [ 자식객체는 힙생성 시 부모객체 힙도 생성
	Child child = new Child(); 		// --> 힙 : Parent,Child 객체 2개
	
	// 3. (자동타입변환)자식객체를 부모객체에 대입???
	Parent parent2 = child; 	// 가능 : 자식객체를 부모객체가 될 수 있다.
	// !! : 자식객체가 부모객체로 대입후에 변환된 부모객체는 자식객체의 멤버 사용할 수 없다.
	...
	// Child child2 = (Chile)parent4; // 실행 과정 오류 발생
	Parent parent3 = new Child(); // 자동 타입 변환 후에 강제 타입 변환 가능하다.
	Child child2 = (Child)parent3; // *단 : 강제 캐스팅을 이용한 형변환 가능
