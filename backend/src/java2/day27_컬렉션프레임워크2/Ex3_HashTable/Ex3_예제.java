...
public class Ex3_예제 {
	...
	public static void main(String[] args) {
		...
		threadA.start(); threadB.start();//두스레드 실행
		
		// main스레드는 다른 두 스레드가 작업(저장)이 끝날 때까지 main스레드 잠시 대기
		try {threadA.join();threadB.join();}catch(Exception e) {} 
		
		System.out.println("map 객체수 : " + map.size());
		System.out.println("map 상태 : " + map); // join()안하면 ... 두 스레드가 저장을 하기 전에 main 스레드 출력 먼저 했기 때문에
	}
}
