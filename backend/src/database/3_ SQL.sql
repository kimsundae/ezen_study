...
select * from member where mname != '에이핑크';		# 6-2. '에이핑크'가 아니면
...
select * from member where mname like '%우%';		# 9. '우'가 포함된 문자
select * from member where mname like '_우_';		# 9. '우'가 두번째에 위치한 세글자
...
select * from member where mphone = null;				# 불가능
...
select * from member order by mdebut desc;	# 데뷔일 필드 기준으로 내림차순
	# 정렬 기준 2개 이상 [ drder by 필드명 정렬기준 , 필드명 정렬기준 ]
    # 첫번째 정렬 후 동일한 키가 있을 경우 동일한 키 중에서 데뷔날짜 오름차순 
select * from member order by mheight desc , mdebut asc;
# 6. 검색 레코드 수 제한 [ limit 레코드 수 , limit 시작레코드번호 , 개수 ]
