...
navigator.geolocation.getCurrentPosition( e => {
	...
	let currnetlat = e.coords.latitude;
	let cuurentlng = e.coords.longitude;


	var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
	    mapOption = { 
	        center: new kakao.maps.LatLng(currnetlat, cuurentlng), // 지도의 중심좌표
	        level: 3 // 지도의 확대 레벨
	    };
	...
	kakao.maps.event.addListener(map, 'click', function(mouseEvent) {        
	    ...
	    plat = latlng.getLat(); // 위도와 경도를 전역변수로 이동후 제품등록 시 사용.
	    plng = latlng.getLng();
	});
});
