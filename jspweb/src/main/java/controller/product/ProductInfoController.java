...
public class ProductInfoController extends HttpServlet {
				...
				if( item.isFormField() ) { System.out.println( item.getString() ); } // .getString() : 해당 요청 input의 value 호출 
					...
					// 6.업로드 경로 + 파일명 [ 조합 ] 
						...
						UUID uuid = UUID.randomUUID();
							// UUID클래스 : 고유성을 보장하는 ID를 만들기 위한 식별자 표준 규약  [ - 하이픈 4개 구역 ]
						String filename = uuid+"-"+item.getName().replaceAll("-", "_");
														// 만약에 파일명에 - 하이픈 존재하면 _언더바로 변경
														// 파일명과 UUID간에 식별하기 위해 구분 - 하이폰 사용하기 때문에
						// UUID[ - - - ] - 파일명
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("findByTop".equals(type)) {		
			...
			json = mapper.writeValueAsString(ProductDao.getInstance().findByLatLng( east, west, south, north));
